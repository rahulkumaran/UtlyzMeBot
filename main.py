...
URL = "https://api.telegram.org/bot{}/".format(TOKEN)		#Default URL to look up to

BOT = telepot.Bot(TOKEN)		#Creating a telegram bot using telepot
...
'''def cricscores(bot,update):
	scores = ''
	r = requests.get('https://sports.ndtv.com/cricket/live-scores')	#Gets HTML of entire page
	soup = BeautifulSoup(r.content,'html.parser')
	#print soup
	score_box = soup.find_all('div',attrs={'class':'team-vs clearfix'})	#Navigating to where the score is available in the page
	#score_box_narrow = score_box.find_all('span')		#Narrowing down search to where the score is avaiable
	scores+="\nThe scores of all matches being played currently is displayed below:\n"
	num=1
	#print score_box_narrow
	for i in score_box:
		s = i.text
		if(s=='\n'):
			continue
			...
			print s
		s.replace("\n","")		#To get the score of all live matches and recently done matches
		scores += str(num)+ ') ' +  s + '\n-------------------------------------------------------\n'
		print scores
		num+=1
